...
void TextureRenderer::draw(int x, int y, int width, int height, const gfx::IRotation& rotation, const gfx::Texture& texture) {
    ...
    if (m_group_data.contains(texture.m_data)) {
        ...
        g.vertices.push_back({ x_to_ndc(m_window, x),       y_to_ndc(m_window, y)        }); // top-left
        g.vertices.push_back({ x_to_ndc(m_window, x+width), y_to_ndc(m_window, y)        }); // top-right
        g.vertices.push_back({ x_to_ndc(m_window, x),       y_to_ndc(m_window, y+height) }); // bottom-left
        g.vertices.push_back({ x_to_ndc(m_window, x+width), y_to_ndc(m_window, y+height) }); // bottom-right
        ...
        RenderGroup group {
            texture,
            {
                { x_to_ndc(m_window, x),       y_to_ndc(m_window, y)        }, // top-left
                { x_to_ndc(m_window, x+width), y_to_ndc(m_window, y)        }, // top-right
                { x_to_ndc(m_window, x),       y_to_ndc(m_window, y+height) }, // bottom-left
                { x_to_ndc(m_window, x+width), y_to_ndc(m_window, y+height) }, // bottom-right
            },
            {
                0u, // top-left
                1u, // top-right
...
void TextureRenderer::flush() {
    ...
    for (auto& [key, value] : m_group_data) {
        auto& [texture, vertices, indices, uvs] = value;
